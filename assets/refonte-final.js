...
function style(){
 if(document.getElementById('ap-v129-style'))return;
 const s=document.createElement('style');s.id='ap-v129-style';s.textContent=`
 ...
 #ap-lang-fixed .ap-lang-select,.ap-lang-picker .ap-lang-select{display:none!important}
 .ap-v129-lang-btn{height:38px;min-width:62px;padding:0 14px;border:1px solid rgba(72,40,68,.18);border-radius:999px;background:#fffaf2;color:#42183d;font:700 13px/1 Lato,sans-serif;display:flex;align-items:center;justify-content:center;gap:6px;cursor:pointer;box-shadow:0 5px 16px rgba(69,30,59,.10)}
 .ap-v129-lang-btn:after{content:'▾';font-size:10px;opacity:.72}.ap-v129-lang-pop{position:fixed;display:none;min-width:170px;padding:8px;background:#fffaf2;border:1px solid rgba(72,40,68,.16);border-radius:16px;box-shadow:0 20px 55px rgba(44,20,40,.23);z-index:999999}.ap-v129-lang-pop.open{display:grid;gap:4px}.ap-v129-lang-pop button{border:0;background:transparent;color:#42183d;text-align:left;border-radius:10px;padding:10px 12px;font:600 13px/1.2 Lato,sans-serif;cursor:pointer;white-space:nowrap}.ap-v129-lang-pop button:hover,.ap-v129-lang-pop button.active{background:#f0e3ef;color:#5b2853}
...
function monthStat(year,month,key){
 const days=[2,6,10,14,18,22,26,28];let net=0,total=0,count=0,strongest=null;
 for(const day of days){
   ...
   for(const x of arr){
     ...
     if(!strongest||force>strongest.force)strongest={force,pol:x.polarite||'mixte',label:x.label||''};
   }
 }
 if(!count)return {v:null,count:0,activity:0,strongest:null};
 return {v:total?Math.max(-1,Math.min(1,net/total)):0,count,activity:total,strongest};
}
...
function renderGraph(){
 ...
 const key=graphYear+'-'+dom.key+'-'+rows.map(r=>r.v===null?'x':r.v.toFixed(2)+':'+r.count).join(',');if(card.dataset.v129===key)return;card.dataset.v129=key;
 ...
 card.querySelectorAll('[data-y]').forEach(b=>b.onclick=()=>{graphYear=Number(b.dataset.y)||y0;card.dataset.v129='';renderGraph()});
 card.querySelectorAll('[data-d]').forEach(b=>b.onclick=()=>{graphDomain=b.dataset.d;card.dataset.v129='';renderGraph()});
}

function applyLanguage(code){
 if(!['fr','en','es','ar'].includes(code))return;localStorage.setItem('astro-lang',code);
 ...
 const b=document.getElementById('ap-v129-lang-btn');if(b)b.textContent=code.toUpperCase();
 document.querySelectorAll('#ap-v129-lang-pop button').forEach(x=>x.classList.toggle('active',x.dataset.lang===code));
}
function ensureLanguageUI(){
 style();
 document.querySelectorAll('.ap-lang-select').forEach(s=>s.style.setProperty('display','none','important'));
 const host=document.getElementById('ap-lang-fixed');if(!host)return;
 if(!document.getElementById('ap-v129-lang-btn')){
   host.innerHTML='';host.style.display='inline-flex';host.style.alignItems='center';
   const btn=document.createElement('button');btn.id='ap-v129-lang-btn';btn.type='button';btn.className='ap-v129-lang-btn';btn.textContent=lang().toUpperCase();host.appendChild(btn);
   btn.onclick=e=>{e.stopPropagation();const pop=document.getElementById('ap-v129-lang-pop');if(!pop)return;const r=btn.getBoundingClientRect();pop.style.top=(r.bottom+8)+'px';pop.style.left=Math.max(8,Math.min(window.innerWidth-pop.offsetWidth-8,r.right-170))+'px';pop.classList.toggle('open')};
 }
 let pop=document.getElementById('ap-v129-lang-pop');if(!pop){pop=document.createElement('div');pop.id='ap-v129-lang-pop';pop.className='ap-v129-lang-pop';const names={fr:'Français',en:'English',es:'Español',ar:'العربية'};['fr','en','es','ar'].forEach(code=>{const b=document.createElement('button');b.type='button';b.dataset.lang=code;b.textContent=code.toUpperCase()+' — '+names[code];if(code===lang())b.classList.add('active');b.onclick=e=>{e.stopPropagation();applyLanguage(code);pop.classList.remove('open')};pop.appendChild(b)});document.body.appendChild(pop)}
}
if(!window.__apV129Outside){window.__apV129Outside=true;document.addEventListener('click',()=>document.getElementById('ap-v129-lang-pop')?.classList.remove('open'))}
...
function run(){renderGraph();ensureLanguageUI();fixStory();fixImages()}
