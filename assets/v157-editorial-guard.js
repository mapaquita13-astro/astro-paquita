...
function featureOf(body){
 ...
 if(t.includes('portrait enfant')||t.includes('child portrait')||t.includes('retrato infantil'))return 'child';
...
- Quand une période succède à une autre, expliquer la transition si les données montrent réellement un changement de tonalité.
...
const ROLE={
 ...
 child:`MODULE « PORTRAIT ENFANT » : lecture adaptée à l'âge, non déterministe et centrée sur l'accompagnement. Structure recommandée : « Ses forces naturelles » → « Ses sensibilités » → « Ce qui l'aide à apprendre » → « Ce qui l'aide à se sentir en sécurité » → « Relation à l'autorité et aux règles » → « Vie sociale » → « Ce qui l'aide à grandir ». Ne jamais figer l'enfant dans une étiquette, prédire son métier, sa réussite scolaire ou son avenir affectif.`,
 forecast_today:`MODULE « PRÉVISIONS — AUJOURD'HUI » : la lecture doit être utile en moins de 20 secondes avant d'être détaillée. Ouvrir par « Votre journée en une phrase ». Puis : « Ce qui ressort aujourd'hui » avec au maximum 1 à 3 domaines réellement actifs ; « Point d'attention » uniquement s'il existe un signal délicat net ; « Comment utiliser la journée » avec une formulation souple réellement liée au signal ; « Ce qui ne ressort pas particulièrement » si certains domaines sont calmes. Ajouter « Par rapport à hier » uniquement si des données d'hier sont fournies. Ajouter « Et demain ? » uniquement si des données de demain sont fournies. Ne jamais inventer un meilleur moment de la journée si aucune donnée horaire n'est fournie. Une journée sans signal remarquable doit pouvoir être décrite comme calme ou stable.`,
 forecast_future:`MODULE « PRÉVISIONS » : c'est le centre de lecture temporelle. AU NIVEAU SEMAINE : vue 7 jours très synthétique, puis 2 ou 3 jours réellement remarquables seulement. AU NIVEAU MOIS : « Le thème du mois » → 2 ou 3 périodes importantes → domaine dominant → « Ce qui change par rapport au mois précédent » uniquement si les données comparatives sont fournies → point d'attention → « Ce qui ne ressort pas ». Si les données le permettent, synthétiser aussi « Ce qui s'ouvre / Ce qui se consolide / Ce qui demande de l'attention » sans forcer trois catégories si l'une d'elles n'a aucun signal. AU NIVEAU TRIMESTRE : raconter les transitions et l'enchaînement des phases, pas juxtaposer trois mois. AU NIVEAU ANNÉE : « À retenir » → « Les fils rouges de l'année » (maximum 3) → phases de l'année → périodes de concrétisation/vigilance → « Ce qui ne ressort pas suffisamment ». Les grandes tendances mois par mois restent une visualisation intégrée aux Prévisions, pas un produit concurrent.`,
