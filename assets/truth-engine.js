/* Astro Paquita — compatibilité V146
    ...
    const wrappedApply=async function(code,msgEl){
      ...
      try{
        if(data&&data.type==='reduction_pourcentage'&&data.checkout_required){
          ...
          if(msgEl){msgEl.textContent=`Code ${promo} validé : -${Number(data.valeur)||0}% sera appliqué au paiement Premium.`;msgEl.style.color='#4ade80';}
        }else if(data){sessionStorage.removeItem('ap-premium-promo-pending')}
      }catch(e){}
      return data;
    };
    wrappedApply.__apV135PromoBridge=true;window.appliquerCodePromo=wrappedApply;
  }
  ...
  if(typeof premiumBase==='function'&&!premiumBase.__apV135PromoBridge){
    const wrappedPremium=async function(){
      ...
      try{
        ...
        if(!resp.ok){if(msg){msg.textContent=data.erreur||'Code promo ou paiement indisponible.';msg.style.color='#f87171'}else alert(data.erreur||'Paiement indisponible pour le moment.');return}
        if(data.url){window.location.href=data.url;return}
        if(msg){msg.textContent='Réponse Stripe invalide.';msg.style.color='#f87171'}
      }catch(e){if(msg){msg.textContent='Erreur de connexion au paiement.';msg.style.color='#f87171'}else alert('Erreur de connexion au paiement.')}
    };
    wrappedPremium.__apV135PromoBridge=true;wrappedPremium.__apV135Base=premiumBase;window.passerPremium=wrappedPremium;
  }
}

function secureMaintenanceBypass(){
  try{const url=new URL(location.href);if(url.searchParams.has('maintenance_preview')){url.searchParams.delete('maintenance_preview');history.replaceState(history.state,'',url.pathname+(url.searchParams.toString()?'?'+url.searchParams.toString():'')+url.hash)}localStorage.removeItem('astro-maintenance-bypass')}catch(e){}
  if(typeof window.verifierMaintenanceSite==='function')setTimeout(()=>{try{window.verifierMaintenanceSite()}catch(e){}},0);
}
function disableLegacyNotifications(){
  ...
  window.apEnableNotifications=async function(){try{localStorage.removeItem('ap-notifications')}catch(e){}document.getElementById('ap-notification-badge')?.remove();return false};
...
function hideQuestionModule(){
  ...
  document.querySelectorAll('#mod-question,#ap-v130-question-card,[onclick*="question" i],[data-feature="question"],[data-module="question"]').forEach(el=>{el.style.setProperty('display','none','important');el.setAttribute('aria-hidden','true')});
}
